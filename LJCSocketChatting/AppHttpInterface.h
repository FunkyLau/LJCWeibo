...
static inline NSString* severURL(kSeverType sType) {
    ...
    dispatch_once(&token, ^{
        ...
        if (sType == kServer_Dev) {
             ...
             * 开发环境---内网地址
             */
            //url = @"http://192.168.200.246:8080/appInterface/";
            url = @"http://192.168.201.136:8080/appInterface/";
...
// 我的产品
#define kQueryOrderList [NSString stringWithFormat:@"%@queryOrderList",SeverURL]    //获取全部订单
#define kCancelProductOrder [NSString stringWithFormat:@"%@updateOrderByOrderId",SeverURL]  //取消订单
