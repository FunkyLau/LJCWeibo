...
- (void)viewDidLoad {
    ...
    [self viewConfig];
    [self initConfig];
    [NS_NOTIFICATION_CENTER addObserver:self selector:@selector(selectedChannelChangedNotbouification:) name:SelectedChannelChangedNotification object:nil];
}

- (void)selectedChannelChangedNotbouification:(NSNotification *)notification
{
...
- (void)viewInit
{
    //self.view.frame = (CGRect){0,65,kScreenWidth,kScreenHeight-65};
    self.view.frame = (CGRect){0,65,kScreenWidth,kScreenHeight-65};
...
//    statusView.backgroundColor = CLEAR_COLOR;
//    [self.view addSubview:statusView];
    
    _navTabBar = [[ZLNavTabBar alloc] initWithFrame:CGRectMake(ZERO_COORDINATE, ZERO_COORDINATE, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT)];
...
//    _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(ZERO_COORDINATE, _navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - _navTabBar.frame.origin.y - _navTabBar.frame.size.height-120)];
    _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(ZERO_COORDINATE, _navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - _navTabBar.frame.origin.y - _navTabBar.frame.size.height-60)];
...
- (void)initConfig
{
    ...
    if ([NSUD objectForKey:SELECTED_CHANNEL_NAMES] == nil) {
        ...
        //_titles = [NSUD objectForKey:SELECTED_CHANNEL_NAMES];
        //_titles =
        [self addChildViewControllerWithTitlesArray:_totalTitles];
    }
    _navTabBar.totalItemTitles = _totalTitles;
    //_navTabBar.selectedItemTitles = _titles;
    _navTabBar.selectedItemTitles = _totalTitles;
}
...
- (void)itemDidSelectedWithIndex:(NSInteger)index
{
    ...
    NSString *selectedChannel = _totalTitles[index];
