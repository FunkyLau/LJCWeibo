...
{
    UIButton *destructiveButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, BUTTON_INTERVAL_HEIGHT, kScreenWidth-BUTTON_INTERVAL_WIDTH*2, BUTTON_HEIGHT)];
...
{
    UIButton *otherButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, BUTTON_INTERVAL_HEIGHT + (postionIndex*(BUTTON_HEIGHT+(BUTTON_INTERVAL_HEIGHT/2))), kScreenWidth-BUTTON_INTERVAL_WIDTH*2, BUTTON_HEIGHT)];
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, BUTTON_INTERVAL_HEIGHT, kScreenWidth-BUTTON_INTERVAL_WIDTH*2, BUTTON_HEIGHT)];
    ...
    cancelButton.backgroundColor = DEFAULT_BACKGROUND_COLOR;
...
- (void)clickOnButtonWith:(UIButton *)button
{
    if (self.isHadDestructionButton == YES) {
        if (self.delegate) {
            if (button.tag == 0) {
                if ([self.delegate respondsToSelector:@selector(didClickOnDestructiveButtonId:)] == YES){
                    [self.delegate didClickOnDestructiveButtonId:self];
                }
            }
        }
    }
    ...
    if (self.isHadCancelButton == YES) {
        if (self.delegate) {
            if (button.tag == self.postionIndexNumber-1) {
                if ([self.delegate respondsToSelector:@selector(didClickOnCancelButtonId:)] == YES) {
                    [self.delegate didClickOnCancelButtonId:self];
                }
            }
        }
    }
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(didClickOnButtonIndex:andId:)] == YES) {
            [self.delegate didClickOnButtonIndex:(NSInteger)button.tag andId:self];
        }
    }
