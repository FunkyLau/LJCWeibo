...
@property (strong, nonatomic) UIImage *imageSaved;
@end
...
- (void)viewDidLoad {
    ...
    [self.closeButton addTarget:self action:@selector(clickCloseBt) forControlEvents:UIControlEventTouchUpInside];
...
- (void)uploadHeadPic:(UIImage *)imageData_PNG{
                ...
                contextInfo:(void *)contextInfo
{
    ...
    if (error != NULL){
        ...
        if ([_delegate respondsToSelector:@selector(chooseImage:)]) {
            [_delegate chooseImage:image];
        }
...
- (void)clickChoseBt:(UIImage *)image{
    //确认选择拍照图片
    _imageSaved = image;
    [self saveImagecompletion:image didFinishSavingWithError:nil contextInfo:nil];
}
