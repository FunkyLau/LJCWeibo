...
- (void)touchImage:(id)sender{
    if ([_delegate respondsToSelector:@selector(didTouchImage:)]) {
        [_delegate didTouchImage:self];
    }
}
...
-(void)setContentImage:(UIImage *)contentImage{
    ...
    UITapGestureRecognizer *recognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(touchImage:)];
    ...
    [btnDelete addTarget:self action:@selector(deletePhotoItem:)forControlEvents:UIControlEventTouchUpInside];
